...
void read_cl_file() {
  ...
  fp = fopen(KERNEL_PATH "/correlation.cl", "r");
  if (!fp) {
...
void cl_mem_init(DATA_TYPE *data, DATA_TYPE *mean, DATA_TYPE *stddev,
  ...
  data_mem_obj =
      clCreateBuffer(clGPUContext, CL_MEM_READ_WRITE,
                     sizeof(DATA_TYPE) * (M + 1) * (N + 1), NULL, &errcode);
  symmat_mem_obj =
      clCreateBuffer(clGPUContext, CL_MEM_READ_WRITE,
                     sizeof(DATA_TYPE) * (M + 1) * (N + 1), NULL, &errcode);
  stddev_mem_obj = clCreateBuffer(clGPUContext, CL_MEM_READ_WRITE,
                                  sizeof(DATA_TYPE) * (M + 1), NULL, &errcode);
  mean_mem_obj = clCreateBuffer(clGPUContext, CL_MEM_READ_WRITE,
                                sizeof(DATA_TYPE) * (M + 1), NULL, &errcode);
  ...
  errcode = clEnqueueWriteBuffer(clCommandQue, data_mem_obj, CL_TRUE, 0,
                                 sizeof(DATA_TYPE) * (M + 1) * (N + 1), data, 0,
                                 NULL, NULL);
  errcode = clEnqueueWriteBuffer(clCommandQue, symmat_mem_obj, CL_TRUE, 0,
                                 sizeof(DATA_TYPE) * (M + 1) * (N + 1), symmat,
                                 0, NULL, NULL);
  errcode =
      clEnqueueWriteBuffer(clCommandQue, stddev_mem_obj, CL_TRUE, 0,
                           sizeof(DATA_TYPE) * (M + 1), stddev, 0, NULL, NULL);
  errcode =
      clEnqueueWriteBuffer(clCommandQue, mean_mem_obj, CL_TRUE, 0,
                           sizeof(DATA_TYPE) * (M + 1), mean, 0, NULL, NULL);
  if (errcode != CL_SUCCESS)
...
void cl_launch_kernel() {
  ...
  size_t oldLocalWorkSize_Kernel1[2], globalWorkSize_Kernel1[2];
  size_t oldLocalWorkSize_Kernel2[2], globalWorkSize_Kernel2[2];
  size_t oldLocalWorkSize_Kernel3[2], globalWorkSize_Kernel3[2];
  size_t oldLocalWorkSize_Kernel4[2], globalWorkSize_Kernel4[2];
  ...
  ///////////////////////////////////////////////
  // Kernel 1.
  oldLocalWorkSize_Kernel1[0] = DIM_LOCAL_WORK_GROUP_KERNEL_1_X;
  oldLocalWorkSize_Kernel1[1] = DIM_LOCAL_WORK_GROUP_KERNEL_1_Y;
  getNewSizes(NULL, oldLocalWorkSize_Kernel1, NULL, localWorkSize_Kernel1, "mean_kernel", 1);
  globalWorkSize_Kernel1[0] = M;
  globalWorkSize_Kernel1[1] = 1;

  // Kernel 2.
  oldLocalWorkSize_Kernel2[0] = DIM_LOCAL_WORK_GROUP_KERNEL_2_X;
  oldLocalWorkSize_Kernel2[1] = DIM_LOCAL_WORK_GROUP_KERNEL_2_Y;
  getNewSizes(NULL, oldLocalWorkSize_Kernel2, NULL, localWorkSize_Kernel2, "std_kernel", 1);
  globalWorkSize_Kernel2[0] = M;
  globalWorkSize_Kernel2[1] = 1;

  // Kernel 3.
  oldLocalWorkSize_Kernel3[0] = DIM_LOCAL_WORK_GROUP_KERNEL_3_X;
  oldLocalWorkSize_Kernel3[1] = DIM_LOCAL_WORK_GROUP_KERNEL_3_Y;
  getNewSizes(NULL, oldLocalWorkSize_Kernel3, NULL, localWorkSize_Kernel3, "reduce_kernel", 2);
  globalWorkSize_Kernel3[0] = M;
  globalWorkSize_Kernel3[1] = N;

  // Kernel 4.
  oldLocalWorkSize_Kernel4[0] = DIM_LOCAL_WORK_GROUP_KERNEL_4_X;
  oldLocalWorkSize_Kernel4[1] = DIM_LOCAL_WORK_GROUP_KERNEL_4_Y;
  getNewSizes(NULL, oldLocalWorkSize_Kernel4, NULL, localWorkSize_Kernel4, "corr_kernel", 1);
  globalWorkSize_Kernel4[0] = M;
  globalWorkSize_Kernel4[1] = 1;
  ///////////////////////////////////////////////

  // Set the arguments of the kernel
  errcode =
      clSetKernelArg(clKernel_mean, 0, sizeof(cl_mem), (void *)&mean_mem_obj);
  errcode |=
      clSetKernelArg(clKernel_mean, 1, sizeof(cl_mem), (void *)&data_mem_obj);
  errcode |=
      clSetKernelArg(clKernel_mean, 2, sizeof(DATA_TYPE), (void *)&float_n);
  ...
  errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel_mean, 1, NULL,
                                   globalWorkSize_Kernel1,
                                   localWorkSize_Kernel1, 0, NULL, NULL);
  if (errcode != CL_SUCCESS)
  ...
  // Set the arguments of the kernel
  errcode =
      clSetKernelArg(clKernel_std, 0, sizeof(cl_mem), (void *)&mean_mem_obj);
  errcode =
      clSetKernelArg(clKernel_std, 1, sizeof(cl_mem), (void *)&stddev_mem_obj);
  errcode |=
      clSetKernelArg(clKernel_std, 2, sizeof(cl_mem), (void *)&data_mem_obj);
  errcode |=
      clSetKernelArg(clKernel_std, 3, sizeof(DATA_TYPE), (void *)&float_n);
  ...
  // Execute the OpenCL kernel
  errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel_std, 1, NULL,
                                   globalWorkSize_Kernel2,
                                   localWorkSize_Kernel2, 0, NULL, NULL);
  if (errcode != CL_SUCCESS)
  ...
  // Set the arguments of the kernel
  errcode =
      clSetKernelArg(clKernel_reduce, 0, sizeof(cl_mem), (void *)&mean_mem_obj);
  errcode = clSetKernelArg(clKernel_reduce, 1, sizeof(cl_mem),
                           (void *)&stddev_mem_obj);
  errcode |=
      clSetKernelArg(clKernel_reduce, 2, sizeof(cl_mem), (void *)&data_mem_obj);
  errcode |=
      clSetKernelArg(clKernel_reduce, 3, sizeof(DATA_TYPE), (void *)&float_n);
  ...
  // Execute the OpenCL kernel
  errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel_reduce, 2, NULL,
                                   globalWorkSize_Kernel3,
                                   localWorkSize_Kernel3, 0, NULL, NULL);
  if (errcode != CL_SUCCESS)
  ...
  // Set the arguments of the kernel
  errcode =
      clSetKernelArg(clKernel_corr, 0, sizeof(cl_mem), (void *)&symmat_mem_obj);
  errcode |=
      clSetKernelArg(clKernel_corr, 1, sizeof(cl_mem), (void *)&data_mem_obj);
...
//  // Execute the OpenCL kernel
//  errcode = clEnqueueNDRangeKernel(clCommandQue, clKernel_corr, 1, NULL,
//                                   globalWorkSize_Kernel4,
//                                   localWorkSize_Kernel4, 0, NULL, NULL);
//  if (errcode != CL_SUCCESS)
//    printf("Error in launching kernel4\n");
//  clEnqueueBarrier(clCommandQue);
//
//  DATA_TYPE val = 1.0;
//  clEnqueueWriteBuffer(clCommandQue, symmat_mem_obj, CL_TRUE,
//                       ((M) * (M + 1) + (M)) * sizeof(DATA_TYPE),
//                       sizeof(DATA_TYPE), &val, 0, NULL, NULL);
...
int main(void) {
  ...
  size_t newSizes[2];
  getNewSizes(oldSizes, NULL, newSizes, NULL, "mean_kernel", 1);
  M = newSizes[0];
  //N = oldSizes[1];
  N = newSizes[0];

  // Kernel 2.
  getNewSizes(oldSizes, NULL, newSizes, NULL, "std_kernel", 1);
  M = newSizes[0];
  //N = oldSizes[1];
  N = newSizes[0];

  // Kernel 3.
  getNewSizes(oldSizes, NULL, newSizes, NULL, "reduce_kernel", 2);
  M = newSizes[0];
  N = newSizes[1];

//  // Kernel 4.
//  getNewSizes(oldSizes, NULL, newSizes, NULL, "corr_kernel", 1);
//  M = newSizes[0];
//  //N = oldSizes[1];
//  N = newSizes[0];
  /////////////////////////
  ...
  errcode = clEnqueueReadBuffer(clCommandQue, symmat_mem_obj, CL_TRUE, 0,
                                ...
                                symmat_outputFromGpu, 0, NULL, NULL);
  if (errcode != CL_SUCCESS)
